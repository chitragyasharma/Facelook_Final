  ...
  <script>
    ...
    async function renderReviews() {
      ...
      try {
        ...
        if (reviews.length === 0) {
           list.innerHTML = summaryHTML + '<div style="padding: 20px; text-align: center; color: var(--tm);">No reviews yet. Write one!</div>';
           return;
        }
    ...
    <div style="display:flex;align-items:center;gap:6px;">
      ...
      <div class="form-section">
        ...
        <div style="background:var(--card);border:1px solid var(--border);border-radius:14px;padding:16px;margin-top:16px;">
          ...
          ${state.coupon ? `<div style="display:flex;justify-content:space-between;margin-bottom:6px;font-size:12px;color:var(--rose-d);"><span>Discount (${state.coupon.code})</span><span>-₹${state.couponDiscount}</span></div>` : ''}
