    ...
    function renderPage(page) {
      ...
      if (page === 'account') {
         if(!state.token) { window.location.hash = '#auth'; return; }
         renderAccount();
      }
    }
    ...
    function productCardHTML(p, extraStyle = '') {
      ...
      return `
    <a href="#product-${p.id}" class="product-card" data-id="${p.id}" style="${extraStyle}; text-decoration:none; color:inherit; display:block;" onclick="openProduct(${p.id})">
    ...
    function selectPaletteShade(index, pid) {
      ...
      if (mainImg) {
        ...
        document.getElementById('detail-emoji').innerHTML = `<img src="${shadeData.image}" id="main-detail-img" style="width:100%;height:100%;object-fit:contain;">`;
      }
    ...
    function openProduct(id) {
      ...
      if (p.images && p.images.length > 1) {
        ...
        thumbGallery.innerHTML = p.images.map((src, idx) =>
      ...
      document.getElementById('detail-shade').innerHTML = 'Shade: <span style="font-weight:600;color:var(--td);">' + p.shade + '</span>';
    ...
    async function finishCheckout() {
      ...
      try {
        ...
        if (state.checkoutData.pay === 'cod') {
          ...
          renderCheckout();
          window.scrollTo(0, 0);
        } else if (state.checkoutData.pay === 'upi') {
          showUpiQrModal(total, order_id);
        } else {
          // Process digital payment with Razorpay
    ...
    function showUpiQrModal(amount, orderId) {
      currentUpiOrderId = orderId;
      document.getElementById('qr-amount').innerText = amount;
      document.getElementById('upi-dynamic-qr').src = `https://api.qrserver.com/v1/create-qr-code/?size=200x200&color=3A2020&data=upi://pay?pa=9910215683@ptyes%26pn=FACELOOK%26am=${amount}`;
      document.getElementById('upi-qr-overlay').style.display = 'block';
      const input = document.getElementById('upi-utr-input');
      if (input) input.value = '';
    }

    async function submitUpiQrPayment() {
      const utr = document.getElementById('upi-utr-input').value;
      if (!utr || utr.length < 5) {
        showToast("Please enter a valid UTR or Transaction ID");
        return;
      }
      
      try {
        await api('/api/checkout/upi', 'POST', { order_id: currentUpiOrderId, utr: utr });
        
        document.getElementById('upi-qr-overlay').style.display = 'none';
        state.cart = [];
        updateBadges();
        state.checkoutStep = 3;
        renderCheckout();
    ...
    async function initApp() {
      ...
      if (initialHash.startsWith('product-')) {
        const pId = parseInt(initialHash.split('-')[1]);
        if (pId) {
          const p = PRODUCTS.find(x => x.id === pId);
          if (p) {
            renderProductDetails(p);
